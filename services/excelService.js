...
async function processExcelAndSendMessages(
  ...
  function emitSocket() {
    resultsSocket.progress = Math.min(
      100,
      Math.round(
        ((resultsSocket.success + resultsSocket.failed) / resultsSocket.total) *
          100
      )
    );
  ...
  if (notifyNumber) {
    try {
      ...
      await sock.sendMessage(`${notifyNumber}@s.whatsapp.net`, {
        text: report,
      });
    } catch (err) {
      logger.error(
    ...
    logger.warn(
      `⚠️ Tidak bisa kirim notifikasi. Session ${sessionId} tidak aktif`
    );
  }
  return results;
}
