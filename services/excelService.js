...
async function processExcelAndSendMessages(
  ...
  inputNumbers = null
    ...
    if (!sock || typeof sock.sendMessage !== 'function') {
      await emitFailure(`Session "${sessionId}" tidak ditemukan atau tidak valid`, rows.length)
      return []
    }

    logger.info(`🚀 Mulai proses blast dengan sessionId: ${sessionId}, total kontak: ${rows.length}`)

    function emitSocket() {
      resultsSocket.progress = Math.min(
        100,
        Math.round(((resultsSocket.success + resultsSocket.failed) / resultsSocket.total) * 100)
      )
      io?.emit('blast-status', resultsSocket)
      logger.info(`📡 Emit status: ${resultsSocket.progress}% | Success: ${resultsSocket.success} | Failed: ${resultsSocket.failed}`)
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!phone || !message) {
        ...
        results.push({ phone, status: 'error', reason: 'Missing phone/message' })
      ...
      try {
        ...
        const [check] = await withTimeout(sock.onWhatsApp(`${phone}@s.whatsapp.net`))

        if (!check?.exists) {
          logger.warn(`❌ Nomor tidak ditemukan di WhatsApp: ${phone}`)
          results.push({ phone, status: 'error', reason: 'Nomor tidak aktif di WhatsApp' })
          resultsSocket.failed++
          emitSocket()
          continue
        }
        ...
        const sentMsg = await withTimeout(sock.sendMessage(`${phone}@s.whatsapp.net`, { text: message }))
        logger.info(`✅ Pesan terkirim ke: ${phone}, messageId: ${sentMsg.key.id}`)
        ...
        const isSessionError = /not connected|disconnected|logout|terminated|conn|closed/i.test(err.message)
        const isTimeoutError = Boom.isBoom(err) && err.output?.statusCode === 408

        logger.error(`❌ Gagal kirim ke ${phone}: ${err.message}`)

        if (isTimeoutError) {
          logger.warn(`⚠️ Timeout saat proses nomor ${phone}, akan di-skip`)
          results.push({ phone, status: 'error', reason: 'Timeout saat verifikasi nomor' })
          resultsSocket.failed++
          emitSocket()
        ...
        if (isSessionError) {
          await emitFailure(`Session WhatsApp terputus saat proses`, rows.length)
          break
        }
      }
    ...
    if(selectTarget !== 'input' && filePath) {
      ...
      logger.info(`🧹 File Excel dihapus: ${filePath}`)
    }
    ...
    if (resultsSocket.status !== 'error') {
      ...
      logger.info(`📦 Campaign selesai. Total: ${rows.length} | Sukses: ${resultsSocket.success} | Gagal: ${resultsSocket.failed}`)
    }

    report = `📦 Campaign selesai. Total: ${rows.length} | Sukses: ${resultsSocket.success} | Gagal: ${resultsSocket.failed}`

    if (notifyNumber) {
      try {
        logger.info(`📲 Mengirim notifikasi ke ${notifyNumber}`)
        await sock.sendMessage(`${notifyNumber}@s.whatsapp.net`, { text: report })
      } catch (err) {
        logger.error(`❌ Gagal kirim notifikasi ke ${notifyNumber}: ${err.message}`)
      }
    } else {
      logger.warn(`⚠️ Tidak bisa kirim notifikasi. Session ${sessionId} tidak aktif`)
    }
    return results
